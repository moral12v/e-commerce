...
export const dashboardDetailLists = async (req, res) => {
  try {
    ...
    let productLists = await VendorProduct.find({ isDeleted: false, isActive: true, status: 'approved' })
      .sort({ createdAt: -1 })
    ...
    let updatedData = await VendorProduct.find({ isDeleted: false, isActive: true, status: 'approved' })
      .sort({ sales: -1 })
      .select(
